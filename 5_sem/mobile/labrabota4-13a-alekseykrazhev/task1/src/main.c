...
const char* readme_txt = "Student <Aleksey Krazhevskiy>, 1623369";
...
static int task3_read(const char *path, char *buf, size_t size, off_t offset, struct fuse_file_info *fi) {
    ...
    if (strcmp(path, "/foo/pwd") == 0) {
        struct stat cat_stat;
        stat("/foo/pwd", &cat_stat);
...
static int task3_readdir(const char *path, void *buf, fuse_fill_dir_t filler, off_t offset, struct fuse_file_info *fi) {
    ...
    if (strcmp(path, "/bar") == 0 && !rmbar) {
        filler(buf, ".", NULL, 0);
        filler(buf, "..", NULL, 0);
        return 0;
    }
    if (strcmp(path, "/baz") == 0) {
        filler(buf, ".", NULL, 0);
        filler(buf, "..", NULL, 0);
...
static int task3_getattr(const char *path, struct stat *stbuf) {
    ...
    else if (strcmp(path, "/bar") == 0 && !rmbar){
...
static int task3_rmdir(const char *path) {
    if (!strcmp(path, "/"))
        ...
        return -ENOTEMPTY;
    else if (!strcmp(path, "/bar") && !rmbar) {
        rmbar = 1;
        return 0;
    } else if (!strcmp(path, "/baz"))
        return -ENOTEMPTY;
