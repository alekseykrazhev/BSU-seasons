...
static const struct inode_operations lab4_file_inode_operations = {
        .lookup = staticfs_lookup,
        .setattr = simple_setattr,
        .getattr = simple_getattr,
};
static const struct inode_operations lab4_dir_inode_operations = {
        .lookup = staticfs_lookup,
};

int lab4_iterate(struct file *file, struct dir_context *ctx) {
...
static const struct file_operations lab4_dir_operations = {
        .iterate      = lab4_iterate,
...
ssize_t lab4_read(struct file *file, char *buf, size_t size, loff_t *offset) {
    char *str;
    struct file *f = NULL;
    struct dentry *dentry = file->f_path.dentry;
    int ino = d_inode(dentry)->i_ino;
...
ssize_t lab4_write(struct file *file, const char *buf, size_t vsize, loff_t *offset) {
...
const struct file_operations lab4_file_operations = {
        .read = lab4_read,
        .write = lab4_write,
};
...
static struct dentry *staticfs_lookup(struct inode *dir, struct dentry *dentry, unsigned int flags) {
    ...
    if (ino >= 0) {
        ...
        switch (mode & S_IFMT) {
            case S_IFREG:
                inode->i_op = &lab4_file_inode_operations;
                inode->i_fop = &lab4_file_operations;
                break;
            case S_IFDIR:
                inode->i_op = &lab4_dir_inode_operations;
                inode->i_fop = &lab4_dir_operations;
                inc_nlink(inode);
                break;
        }
    }
...
static void lab4_put_super(struct super_block *sb) {
    printk(KERN_INFO
    "aufs super block destroyed\n");
}

static struct super_operations const lab4_super_ops = {
        .put_super = lab4_put_super,
};
...
static int aufs_fill_sb(struct super_block *sb, void *data, int silent) {
    ...
    switch ((S_IFDIR | S_IRWXUGO) & S_IFMT) {
        case S_IFREG:
            root->i_op = &lab4_file_inode_operations;
            root->i_fop = &lab4_file_operations;
            break;
        case S_IFDIR:
            root->i_op = &lab4_dir_inode_operations;
            root->i_fop = &lab4_dir_operations;
            inc_nlink(root);
            break;
    }
...
static struct dentry *lab4_mount(struct file_system_type *type,
                                        int flags,
                                        char const *dev,
                                        void *data) {
