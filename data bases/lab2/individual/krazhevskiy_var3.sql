...
CREATE TABLE IF NOT EXISTS `mydb`.`Event` (
  ...
  `Status` ENUM ('Запланировано', 'Проводится', 'Отложено', 'Отменено') NOT NULL,
  ...
  CONSTRAINT `fk_Event_Place`
    ...
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
    CHECK ( Event.start_date >= CURDATE() and Event.end_date > Event.start_date ))
ENGINE = InnoDB;
